...
typedef std::uint32_t  WORD;             // 32-bit word, change to "long" for 16-bit machines
...
// Batch SHA-256 function for the miner
#ifdef __cplusplus
extern "C" {
#endif
void sha256n_batch_hash(BYTE* input_data, size_t input_size, BYTE* output_data, int batch_count);
#ifdef __cplusplus
}
#endif
...
__device__ void mycpy12(WORD *d, const WORD *s) {
#pragma unroll 3
    for (int k=0; k < 3; k++) d[k] = s[k];
}

__device__ void mycpy16(WORD *d, const WORD *s) {
#pragma unroll 4
    for (int k=0; k < 4; k++) d[k] = s[k];
}

__device__ void mycpy32(WORD *d, const WORD *s) {
#pragma unroll 8
    for (int k=0; k < 8; k++) d[k] = s[k];
}

__device__ void mycpy44(WORD *d, const WORD *s) {
#pragma unroll 11
    for (int k=0; k < 11; k++) d[k] = s[k];
}

__device__ void mycpy48(WORD *d, const WORD *s) {
#pragma unroll 12
    for (int k=0; k < 12; k++) d[k] = s[k];
}

__device__ void mycpy64(WORD *d, const WORD *s) {
#pragma unroll 16
